/* ============================================================
   ...
   ------------------------------------------------------------
   CARA KERJA:
   1. Admin klik tombol "Wawancara" di panel → dapat link
   2. Pewawancara buka link → isi form → kirim
   3. PDF otomatis dibuat & upload ke GAS
   4. Muncul di panel admin (ter-group dengan kandidat)
...
(function () {
  ...
  /* ============================================================
     DETEKSI MODE WAWANCARA (dari URL)
     ============================================================ */
  const url = new URL(window.location.href);
  const isInterviewMode = url.searchParams.get('interview') === '1';

  if (!isInterviewMode) return; // Keluar kalau bukan mode wawancara
  ...
  async function uploadToGAS(pdfBlob, interviewer) {
    ...
    const payload = {
      action: 'upload',        // pakai action=upload biasa
      deviceId: 'interview_' + Date.now(),
      filename: filename,
      name: candidateName,      // HARUS sama agar ter-group dengan kandidat
