  ...
  app.innerHTML = `
    <div class="kraeplin-instruction">
      ${renderTestPageHeader({
        ...
        title: 'Instruksi Tes Kraeplin',
        subtitle: 'Baca instruksi singkat berikut sebelum memulai tes.'
      })}
...
function renderKraeplinBoard() {
  ...
  let html = `
    <div class="card kraeplin-card${dangerEffect}" aria-hidden="${(!appState.kraeplinStarted && !isSelesai) ? 'true' : 'false'}">
      ${renderTestPageHeader({
        eyebrow: 'PERFORMANCE ASSESSMENT',
        title: tests.KRAEPLIN.name,
        subtitle: tests.KRAEPLIN.description,
        showLogo: true,
        backLabel: 'Kembali'
      })}

      <div style="text-align:center;margin:6px 0 4px;">
        <p style="margin:0;font-size:.92rem;color:#64748b;">Waktu per kolom: <strong style="color:#4f46e5;"><span id="timer-desc">${appState.timeLeft || 15}s</span></strong></p>
        ${label}
      </div>
